...
int main() {
    ...
    // รับข้อมูลสำหรับบุคคลที่ 1
    printf("Person 1: ");
    // ใช้ ' ' (ช่องว่าง) ก่อน %c ใน scanf เพื่อข้าม whitespace ที่อาจหลงเหลืออยู่ใน buffer
    // ใช้ '*' เป็นตัวคั่นตามที่โจทย์กำหนด
    scanf(" %c*%d*%f*%f*%c", &name1, &age1, &height1, &weight1, &grade_code1);

    // รับข้อมูลสำหรับบุคคลที่ 2
    printf("Person 2: ");
    scanf(" %c*%d*%f*%f*%c", &name2, &age2, &height2, &weight2, &grade_code2);

    // รับข้อมูลสำหรับบุคคลที่ 3
    printf("Person 3: ");
    ...
    // พิมพ์ข้อมูลของแต่ละบุคคลพร้อมการจัดตำแหน่งในตาราง
